...
class ShowGalaxynewPage extends AbstractGamePage
{
    ...
    public function show()
    {
        ...
        $sql = 'SELECT p.*, u.id, u.urlaubs_modus, u.username FROM %%PLANETS%% p, %%USERS%% u WHERE p.id_owner != "" AND p.id_owner = u.id';
        $res = $db->select($sql, []);
        
        
        /*for($x=1;$x<=$config->max_galaxy;$x++){
            for($y=1;$y<=$config->max_system;$y++){
                $qry = 'SELECT COUNT(id) as anz FROM %%PLANETS%% WHERE galaxy ='.$x.' AND system = '.$y." LIMIT 1";
                $all = $db->select($qry,[]);
                $res[$x][$y]["count"] = $all[0]["anz"];
            }
        }
        ...
        $this->tplObj->loadscript('galaxy.js');
        $this->assign(["playerdata" => $res, "x" => $config->max_galaxy, "y" => $config->max_system, "uni" => $config->uni]);
        $this->display('page.galaxynew.default.tpl');
    }
